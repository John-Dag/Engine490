...
public class SwordSpawn extends StaticWeapon {
	public SwordSpawn() {
		super();
	}
	
	public SwordSpawn(Vector3 position, int id, boolean isActive, boolean isRenderable, boolean isDecalFacing, Model model) {
		super(position, id, isActive, isRenderable, isDecalFacing, model);
	}
	
	@Override
	public StaticWeapon spawn() {
		SwordSpawn sword = new SwordSpawn(this.getPosition().cpy(), 1, true, true, true, Assets.manager.get("sword.g3db", Model.class));
